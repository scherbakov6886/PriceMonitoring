...
HEADLESS           = False  # False — показывать браузер при отладке
# ──────────────────────────────────────────────
...
# Категория Пирожки выводится в колонки A-D (как пицца), а не H-J
...
SPECIAL_CATS_MAP = {
    ...
    "напо":    "напої",
    "напій":   "напої",
    "пиріж":   "пиріжки",   # → колонки A-D вместе с пиццей
    "пиріг":   "пиріжки",
    "пирог":   "пиріжки",
}
...
# ═══════════════════════════════════════════════════════════════════
# PLAYWRIGHT: выбор города на dono.in.ua (алгоритм IQPizza)
# ═══════════════════════════════════════════════════════════════════

async def select_city(page, city: str) -> bool:
    """
    Выбор города на dono.in.ua — тот же алгоритм что и IQPizza (платформа EateryClub):
    1. Клик «Самовывоз»
    2. Выбор города через select или дропдаун
    3. Выбор первого адреса
    4. Подтверждение адреса
    """
    city_display = CITY_DISPLAY[city]
    print(f"\n  [1/4] Кликаю «Самовывоз»...")
    ...
    try:
        ...
        if await btn.count() > 0:
            ...
            if await self_pickup.count() > 0:
                ...
                await page.wait_for_timeout(1500)
                print(f"        ✅ Кликнул «Самовывоз» (text locator)")
    except Exception as e:
        print(f"        ⚠️  Самовывоз: {e}")
    ...
    # Попытка 2: кастомный дропдаун (как в IQPizza)
    if not selected:
        try:
            # Открываем дропдаун
            field = page.locator(
            ...
            if await field.count() > 0 and await field.is_visible():
                ...
                # Ищем по тексту "Оберіть місто" или текущий город
            ...
            print(f"        ⚠️  Триггер дропдауна: {e}")

        # Ищем нужный город в опциях
        try:
            found = False
            for selector in [
    ...
    # Закрываем дропдаун если ещё открыт
    ...
    try:
        # Ждём появления карточек адресов
        await page.wait_for_selector(
        ...
        # Запасной вариант — ищем по названию заведения
        if not addr_ok:
            for brand_text in ["DONO", "Dono"]:
                ...
                if await card.count() > 0 and await card.is_visible():
                    ...
                    await page.wait_for_timeout(600)
                    addr_ok = True
                    print(f"        ✅ Первый адрес (по тексту '{brand_text}')")
                    break

        if not addr_ok:
            print(f"        ⚠️  Адрес не выбран!")
    ...
    try:
        ...
        print(f"        ✅ Адрес подтверждён! URL: {page.url}")
        return True
    except Exception as e:
        print(f"        ⚠️  Подтверждение: {e}")

        # Если кнопка "Подтвердить" не найдена — возможно адрес уже выбран
        # Проверяем URL — может уже на странице меню
        current_url = page.url
        if any(x in current_url for x in ["menu", "catalog", "pizza", "category"]):
            print(f"        ✅ Уже на странице меню: {current_url}")
            return True
...
# ═══════════════════════════════════════════════════════════════════
# PLAYWRIGHT: доп. ингредиенты из секции «Додатки»
# ═══════════════════════════════════════════════════════════════════

def _clean_name(name: str) -> str:
    name = name.strip().rstrip('.')
    # Убираем промо-теги если они попали в название
...
async def get_dodatky_from_card(page, card_locator) -> list:
    """
    Кликает по карточке, находит секцию «Додатки» и собирает
    все добавки с ценой.
    """
    dodatky = []
    try:
        ...
        # Раскрываем секцию «Додатки» если свёрнута
        ...
        while i < len(lines):
            ...
            # Цена в той же строке: "Название + 59"
        ...
        for idx in range(card_count):
            ...
            try:
                ...
                if store_key == "піца" and not dodatky_collected:
                    print(f"    → Собираю Додатки из: {name}")
                    dod = await get_dodatky_from_card(page, card)
                    item["dodatky"] = dod
                    dodatky_collected = True
                    if dod:
                        print(f"      ✅ {len(dod)} Додатков: {', '.join(d['name'] for d in dod[:5])}")
                    else:
                        print(f"      ⚠️  Додатки не найдены")
...
# ═══════════════════════════════════════════════════════════════════
# АРХИВ: загрузка предыдущего мониторинга
# ═══════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════
# EXCEL: заполнение шаблона Dono (10 колонок A-J)
# ═══════════════════════════════════════════════════════════════════
...
def fill_excel(all_data: dict, template_path: str) -> str:
    ...
    # ── Пирожки: отдельная база, выводится в A-D после пицц ─────────
    pirozhky_db = {}
    for city in CITIES:
    ...
    # ── Спецкатегории (H-J): Салаты, Соусы, Кофе, Напитки ──────────
    spec_label_map = {
        "6. Салати": "салати",
        "7. Соуси":  "соуси",
        "8. Кава":   "кава",
    ...
    # ── Поток строк: пицца (A-D) → Пирожки (A-D) ───────────────────
    # Тип строки: "pizza_cat" | "pizza" | "pirozhky_cat" | "pirozhky"
    stream_a = []
    for cat in tmpl_cats:
        # tmpl_cats теперь содержит и "5. Пиріжки" — пропускаем здесь
    ...
    # Блок Пирожки (заголовок + позиции) в конце A-D потока
    ...
    for row_type, lbl, pname, pcities in stream_a:
        ...
        # ── Колонки H-J: Салаты/Соусы/Кофе/Напитки ──────────────────
...
def send_telegram(file_path: str):
    if not TELEGRAM_BOT_TOKEN:
        ...
        return
    caption = (
        f"📊 *Мониторинг цен Dono*\n"
        f"🗓 {datetime.now().strftime('%d.%m.%Y %H:%M')}\n"
        f"🍕 Самовывоз — Днепр + Павлоград"
    )
...
async def main():
        ...
        for city in CITIES:
            ...
            try:
                ...
                for attempt in range(1, MAX_RETRIES + 1):
                    try:
                        # ── Всегда открываем главную без slug города ─────────
                        await page.goto(
                        ...
                        # Проверяем доступность сайта (не технические работы)
                        ...
                        if len(page_text.strip()) < 50:
                            raise Exception("Пустая страница")
                ...
                # ── Выбор города через алгоритм IQPizza ─────────────────────
                ok = await select_city(page, city)
                if not ok:
                    print(f"  ⚠️  Город/адрес не подтверждён для {city_label}!")

                # Ждём перехода на меню
                try:
                    ...
                    # Пробуем найти меню через навигацию
                    for menu_text in ["Меню", "Піца", "Pizza", "Каталог"]:
                        try:
                            ...
                            if await menu_link.count() > 0:
                                ...
                                await page.wait_for_timeout(2000)
                                print(f"  ✅ Перешёл через '{menu_text}': {page.url}")
                                break
                        except Exception:
                            pass
    ...
    print(f"\n  Всего пицц: {total}")
    if total == 0:
        print("  ⚠️  Данные не найдены! Проверьте debug_dono_menu_*.png")
        return
